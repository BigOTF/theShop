...
const shoeMainSection = () => {
    clothSection.innerHTML = clothData.map((data) => {
        return (
            `<div class='card'>
...
const generateMainCartPage = () => {
    if(cartData.length !== 0) {
        cartDataDisplay.innerHTML = cartData.map((data) => {
            const searchId = clothData.find((y) => y.id === data.id) || foodData.find((x) => x.id === data.id) || shoeData.find((x) => x.id === data.id)
            return (
               `<div class='sideBar'>
...
const subTotalBill = () => {
    const subtotal = cartData.map((x) => {
        const searchId = clothData.find((data) => data.id === x.id) || foodData.find((data) => data.id === x.id) || shoeData.find((data) => data.id === x.id)
        return (
